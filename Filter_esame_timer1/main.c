    ...
    while(1){
        ...
        Period_Elapsed=0;
                if(digitalRead(52)==HIGH){//normalmente è LOW , SE PREMO ALLORA è HIGH
                    ...
                    /*PHASE_RED_DUTY=ADC_Sample+Red_Duty;
                    PHASE_BLUE_DUTY=ADC_Sample+Blue_Duty;
                    PHASE_GREEN_DUTY=ADC_Sample+Green_Duty;
                    digitalWrite(39,LOW);//39 BLUE
                    GpioCtrlRegs.GPAMUX1.bit.GPIO0=1;
                    EPwm1Regs.CMPA.half.CMPA=ADC_Sample;
                    GpioCtrlRegs.GPAMUX1.bit.GPIO1=1;
               EPwm1Regs.CMPB=ADC_Sample;
                    GpioCtrlRegs.GPAMUX1.bit.GPIO2=1;
               EPwm2Regs.CMPA.half.CMPA=ADC_Sample;
                    GpioCtrlRegs.GPAMUX1.bit.GPIO3=1;
               EPwm2Regs.CMPB=ADC_Sample;*/}
                else{digitalWrite(34,1);
                digitalWrite(39,HIGH);
                    Sin_to_Display=(int)(1000*Sin_Red_Angle);
                    Num_to_Digits(speed, &Digit_1, &Digit_2, &Digit_3, &Digit_4);
                    /*
                    DIGIT1_ON=ADC_Sample-3662;
                    DIGIT2_ON=ADC_Sample-3662;
                    DIGIT3_ON=ADC_Sample-3662;
                    DIGIT4_ON=ADC_Sample-3662;
*/
